...
class TestCreateSubdivisions(ApiClients):
    ...
    def test_get_subdivision_by_id(self):
        status_code, response_data, response_url = self.subdivisions.get_subdivision_by_id(headers=headers_sender)
        self.response.assert_status_code(status_code, response_data, response_url, expected_code=200)
...
class TestCreateRbacs(ApiClients):
    ...
    def test_get_rbacs(self):
        status_code, response_data, response_url = self.rbacs.get_rbacs(headers=headers_sender)
        self.response.assert_status_code(status_code, response_data, response_url, expected_code=200)
    ...
    def test_post_rbac(self):
        ...
        # выполнение теста
        status_code, response_data, response_url = self.rbacs.post_rbac(headers=headers_sender)
        self.response.assert_status_code(status_code, response_data, response_url, expected_code=200)

    def test_get_rbac_for_subdivision_job_title(self):
        status_code, response_data, response_url = self.rbacs.get_rbac_for_subdivision_job_title(headers=headers_sender)
        self.response.assert_status_code(status_code, response_data, response_url, expected_code=200)

    def test_put_rbac(self):
        status_code, response_data, response_url = self.rbacs.put_rbac(headers=headers_sender)
        self.response.assert_status_code(status_code, response_data, response_url, expected_code=200)

    def test_delete_rbac(self):
        status_code, response_data, response_url = self.rbacs.delete_rbac(headers=headers_sender)
        self.response.assert_status_code(status_code, response_data, response_url, expected_code=200)
