...
class TestCreateEmployee(ApiClients):
    ...
    def test_get_countries(self):
        status_code = self.employees.get_countries(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_get_country(self):
        status_code = self.employees.get_country(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_get_employee_personal_info(self):
        status_code = self.employees.get_employee_personal_info(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    @pytest.mark.parametrize('path', ['&WorkingOnly=true', '&WorkingOnly=false'])
    def test_get_employees(self, path):
        status_code, response_data = self.employees.get_employees(path, headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
    ...
    def test_get_employee_certificates(self):
        status_code, response_data = self.employees.get_employee_certificates(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_get_employee_by_id(self):
        status_code = self.employees.get_employee_by_id_current_user(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_post_employee(self):
        status_code = self.employee_saga.post_employee_saga(headers=headers_sender, resident=True)
        assert_that(status_code).is_equal_to(200)

    def test_get_employee_saga(self):
        status_code, created_employee_id = self.employee_saga.get_employee_saga_with_saga_id(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
        assert_that(created_employee_id).is_not_empty()

    def test_put_employee(self):
        self.employees.get_employee_by_id_created_user(headers=headers_sender)
        status_code = self.employees.put_created_employee(headers=headers_sender, resident=True)
        assert_that(status_code).is_equal_to(200)

    def test_delete_employee(self):
        status_code = self.employees.delete_employee(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_get_job_title(self):
        status_code = self.job_title.get_job_title(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_put_avatar_url(self):
        status_code = self.users.put_avatar_url(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_delete_avatar(self):
        status_code = self.users.delete_avatar(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
...
class TestCreateNotResidentEmployee(ApiClients):
    ...
    def test_post_employee_not_resident(self):
        status_code = self.employee_saga.post_employee_saga(headers=headers_sender, resident=False)
        assert_that(status_code).is_equal_to(200)
...
class TestCancelCertificateIssueLocal(ApiClients):
    ...
    def test_post_request_certificate_local(self):
        ...
        status_code = self.uueses.post_request_certificate(headers=headers_sender, issue_type='Local')
        assert_that(status_code).is_equal_to(200)

    def test_get_request_certificate_type(self):
        status_code = self.uueses.get_request_certificate_type(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_post_request_certificate_passport(self):
        status_code, response_data = self.uueses.post_request_certificate_passport(headers=headers_form_data_sender)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data['DownloadedFiles']).is_equal_to(1)

    def test_delete_request_certificate_passport(self):
        status_code, response_data = self.uueses.delete_request_certificate_passport(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
    ...
    def test_get_request_certificate_with_id_local(self):
        status_code, response_data = self.uueses.get_request_certificate_with_id(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
    ...
    def test_get_personal_info_with_certificate_id_local(self):
        status_code = self.uueses.get_personal_info_with_certificate_id(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_post_cancel_request_certificate_local(self):
        status_code, response_data = self.uueses.post_cancel_request_certificate(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')
...
class TestCertificateIssueLocal(ApiClients):
    ...
    def test_post_accept_personal_info_local(self):
        status_code, response_data = self.uueses.post_accept_personal_info(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')

    def test_post_signature_blank_local(self):
        status_code, response_data = self.uueses.post_signature_blank(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')

    def test_get_certificate_blank_local(self):
        status_code = self.uueses.get_certificate_blank(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)

    def test_post_confirm_application_local(self):
        status_code, response_data = self.uueses.post_confirm_application(headers=headers_sender)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')
...
class TestCertificateIssueGosuslugi(ApiClients):
    ...
    def test_post_request_certificate_gosuslugi(self):
        ...
        # создаем запрос на выпуск унэп
        status_code = self.uueses.post_request_certificate(headers=headers_recipient, issue_type='Gosuslugi')
        assert_that(status_code).is_equal_to(200)

    def test_get_request_certificate_with_id_gosuslugi(self):
        status_code, response_data = self.uueses.get_request_certificate_with_id(headers=headers_recipient)
        assert_that(status_code).is_equal_to(200)
    ...
    def test_get_personal_info_with_certificate_id_gosuslugi(self):
        status_code = self.uueses.get_personal_info_with_certificate_id(headers=headers_recipient)
        assert_that(status_code).is_equal_to(200)

    def test_post_accept_personal_info_gosuslugi(self):
        status_code, response_data = self.uueses.post_accept_personal_info(headers=headers_recipient)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')

    def test_post_signature_blank_gosuslugi(self):
        status_code, response_data = self.uueses.post_signature_blank(headers=headers_recipient)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')

    def test_post_confirm_application_gosuslugi(self):
        status_code, response_data = self.uueses.post_confirm_application(headers=headers_recipient)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')
...
class TestCertificateIssueGosuslugiNotResident(ApiClients):
    ...
    def test_post_request_certificate_gosuslugi_not_resident(self):
        ...
        # создаем запрос на выпуск унэп
        status_code = self.uueses.post_request_certificate(headers=headers_not_resident, issue_type='Gosuslugi')
        assert_that(status_code).is_equal_to(200)

    def test_post_accept_personal_info_gosuslugi_not_resident(self):
        status_code, response_data = self.uueses.post_accept_personal_info(headers=headers_not_resident)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')

    def test_post_signature_blank_gosuslugi_not_resident(self):
        status_code, response_data = self.uueses.post_signature_blank(headers=headers_not_resident)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')

    def test_post_confirm_application_gosuslugi_not_resident(self):
        status_code, response_data = self.uueses.post_confirm_application(headers=headers_not_resident)
        assert_that(status_code).is_equal_to(200)
        assert_that(response_data).is_equal_to('True')
